...
- ## Happy path: correct input produces correct output/redirect/template
- ## Auth guard: unauthenticated requests to protected routes return 302 to /login or 401
- ## Validation errors: missing fields, invalid data, duplicate entries return appropriate errors
- ## DB side effects: after a write operation, query the DB to confirm the record was created/updated/deleted
- ## HTTP semantics: correct status codes (200, 201, 302, 400, 404, etc.)
- ## Template rendering: response contains expected HTML landmarks or text
- ## Edge cases: empty strings, very long input, SQL injection attempts (parameterized queries should handle these safely)
## Code Quality Rules
...
Workflow
Clarify the spec: If the feature description is ambiguous, ask 1–2 focused questions before writing tests. Do not invent behavior.
Identify test scope: List all behaviors to test before writing any code.
Write fixtures first: Define or reuse app, client, auth_client at the top of the file.
Write tests systematically: Cover the checklist above for each behavior.
Self-review: Before outputting, verify:
...
Output the complete test file: Always output the full tests/test_<feature>.py file, ready to run with pytest.
...
## A brief test plan (bulleted list of what will be tested and why)
The complete test file in a fenced ```python code block
A run command showing exactly how to execute the new tests
